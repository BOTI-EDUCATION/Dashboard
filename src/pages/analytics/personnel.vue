...
onMounted(async() => {
  await axiosIns.get('structurePersonnel/').then(res=>{
    ...
    inscriptionsGenderValues.push( Number(res.data.ins_by_gender[1].count_eleves) , Number(res.data.ins_by_gender[0].count_eleves))
    for (let i = 0; i < res.data.ins_by_site.length; i++) {
      ...
      inscriptionsSiteValues.push(Number(res.data.ins_by_site[i].count_eleves))
    }
    ...
    ensGenderValues.push( Number(res.data.ens_by_gender[1].count_ens) , Number(res.data.ens_by_gender[0].count_ens))
  ...
  await axiosIns.get('getInscriptionsByCycles/').then(res=>{
    ...
    inscriptionsCicleLabels.push('Préscolaire', 'Primaire', 'Collége' , 'Lycée')
  }).catch(err=>{
    console.log(err)
  })
  loaded.value = true
  ...
  <VRow>
    ...
    >
      <VCard>
        <VCardItem class="d-flex flex-wrap justify-space-between gap-4">
          <VCardTitle>
            Inscriptions par genre ( FILLES / GARCONS )
          </VCardTitle>
        </VCardItem>
        <VCardText>
          <PersonnelDonutChart
